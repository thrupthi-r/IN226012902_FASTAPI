...
# ── Task 6  Combined Search + Sort + Pagination ─────────
...
def browse_products(
    ...
    return {'keyword': keyword,'sort_by': sort_by,'order': order,'page': page,
            'limit': limit,'total_found': total,'total_pages': -(-total // limit),
...
def get_orders_paged(
    ...
    return {
        'page': page,'limit': limit,'total': len(orders),
        'total_pages': -(-len(orders) // limit),'orders': orders[start:end],}
